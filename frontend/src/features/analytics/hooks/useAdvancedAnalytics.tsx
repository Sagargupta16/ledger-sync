...
const calculateCategoryBudgetStatus = (transactions: Transaction[], budgets: Record<string, number>) => {
  ...
  groups.forEach((txns, key) => {
    if (txns.length >= 2) {
      ...
      recurring.push({
        ...
        count: txns.length,
        nextExpected,
        isMonthly: frequencyLabel === "monthly" || frequencyLabel === "bi-weekly" || frequencyLabel === "weekly"
      });
    }
  });
...
const calculateDayOfMonthPattern = (transactions: Transaction[]) => {
  // Simplified stub - returns empty array
  return [];
};
...
const calculateIncomeStability = (transactions: Transaction[]) => {
  // Simplified stub - returns basic stability
  return { rating: "Moderate", score: 50 };
};

const calculateMonthlyHealthRatio = (transactions: Transaction[]) => {
  // Simplified stub - returns empty array
  return [];
};
...
const calculateGoalProgress = (current: number, goal: number, monthlyRate: number) => {
  ...
  return { current, goal, percentage, monthsToGoal: monthlyRate > 0 ? (goal - current) / monthlyRate : 0 };
};
