...
- **`api/`** - FastAPI routers. Each file is a router module (auth, oauth, upload, transactions, analytics, analytics_v2, calculations, preferences, account_classifications, exchange_rates, rates, stock_price, meta, reports, ai_chat, ai_tools, ai_usage). `rates.py` serves `/api/rates/instruments` from `config/instrument_rates.json` (EPF/PPF/NPS rates with `effective_from` + `source_url` metadata). Routers are registered in `main.py`. All endpoints require JWT auth via `get_current_user` dependency from `deps.py`. `oauth.py` handles Google/GitHub authorization code exchange. `preferences.py` includes salary-structure, rsu-grants, growth-assumptions, and ai-config endpoints (including `PATCH /api/preferences/ai-config/mode` to switch between `app_bedrock` and `byok`). `ai_chat.py` proxies Bedrock non-streaming calls (required because Bedrock needs SigV4 auth and doesn't support CORS); branches on `ai_mode` to use either the server's bearer token (app mode) or the user's stored encrypted key (BYOK). `ai_tools.py` exposes `/api/ai/tools` (list) and `/api/ai/tools/execute` (run) for the 15 read-only user-scoped tools the chatbot invokes. `ai_usage.py` records every LLM round-trip in `ai_usage_log` and returns today/MTD/all-time rollups.
- **`core/`** - Business logic. `sync_engine.py` orchestrates imports -- `import_rows()` for JSON uploads from the frontend, `import_file()` for CLI file imports. `reconciler.py` handles deduplication via SHA-256 hashing. `calculator.py` and `analytics_engine.py` compute financial metrics (with module-level helpers extracted to `_analytics_helpers.py`). `query_helpers.py` provides shared SQL aggregation helpers (`income_sum_col`, `expense_sum_col`, `build_transaction_query`) used by both `calculations.py` and `analytics.py`. `analytics/cohort.py` computes the `cohort_spending` rollup (avg expense by day-of-week / day-of-month / month-of-year, occurrence-correct divisors) on each upload. `insights.py` generates smart financial insights. `report_generator.py` builds exportable reports. `time_filter.py` handles date range/fiscal year filtering logic. `encryption.py` provides AES-256-GCM encrypt/decrypt for API keys (PBKDF2-HMAC-SHA256 with per-ciphertext random salt). `core/auth/` handles JWT token creation/verification.
