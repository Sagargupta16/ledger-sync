...
- **`api/`** - FastAPI routers. Each file is a router module (auth, oauth, upload, transactions, analytics, analytics_v2, calculations, preferences, account_classifications, exchange_rates, stock_price, meta, reports, ai_chat, ai_tools, ai_usage). Routers are registered in `main.py`. All endpoints require JWT auth via `get_current_user` dependency from `deps.py`. `oauth.py` handles Google/GitHub authorization code exchange. `preferences.py` includes salary-structure, rsu-grants, growth-assumptions, and ai-config endpoints (including `PATCH /api/preferences/ai-config/mode` to switch between `app_bedrock` and `byok`). `ai_chat.py` proxies Bedrock non-streaming calls (required because Bedrock needs SigV4 auth and doesn't support CORS); branches on `ai_mode` to use either the server's bearer token (app mode) or the user's stored encrypted key (BYOK). `ai_tools.py` exposes `/api/ai/tools` (list) and `/api/ai/tools/execute` (run) for the 15 read-only user-scoped tools the chatbot invokes. `ai_usage.py` records every LLM round-trip in `ai_usage_log` and returns today/MTD/all-time rollups.
...
- **`pages/`** - 25 page components (24 feature pages + `MorePage` for phone grid nav), all lazy-loaded via `React.lazy` for code splitting. Pages import directly (no barrel re-export). **Structure convention**: multi-file pages use kebab-case directories (`bill-calendar/`, `year-in-review/`, `tax-planning/`, `trends-forecasts/`, `comparison/`, `goals/`, `income-expense-flow/`, `settings/`, `subscription-tracker/`) -- each containing `PageName.tsx` + `use<Page>.ts` + `types.ts` + `*utils.ts` + `components/` subfolder. Single-file pages use PascalCase (`DashboardPage.tsx`, `BudgetPage.tsx`, etc.). Settings uses `sections/` (instead of `components/`) because "section" is the domain term. `settings/sectionPrimitives.tsx` provides shared Section/FieldLabel/FieldHint primitives. See [`docs/PAGES.md`](docs/PAGES.md) for a data-focused catalog of what each page shows.
...
- **Mobile navigation**: Phone viewports (`<lg` = <1024px) get a **bottom tab bar** (`frontend/src/components/layout/MobileTabBar.tsx`) with four tabs -- Home / Txns / Flow / More -- plus a `/more` grid page (`MorePage.tsx`) listing every other route. Desktop keeps the sticky sidebar unchanged. `AppLayout` reserves 68px + safe-area-bottom padding on phone for the tab bar; the chat widget floats above it. Every sticky header (`PageHeader`, `HomePage` header) bakes `env(safe-area-inset-top)` into its padding so titles clear the iOS notch in standalone mode. `h-screen` is forbidden in layout primitives -- use `h-dvh` so viewport height tracks the address-bar toggle.
