...
MONTHLY_EXPENSES: list[tuple[str, str, str, tuple[int, int], list[str]]] = [
    ...
    (
        ...
        "HDFC Bank",
        (3000, 6000),
        ["BigBasket", "DMart", "Zepto groceries"],
    ),
    ...
    (
        ...
        "ICICI Credit Card",
        (500, 2500),
        ["Swiggy order", "Zomato", "Dinner out"],
    ),
...
def seed_demo_data(db: Session, user: User) -> int:
        ...
        for month in range(1, last_month + 1):
            ...
            transactions.append(
                _make_txn(
                    ...
                    f"Salary credited - {dt.strftime('%b %Y')}",
                    len(transactions),
                )
            )
            ...
            if month == 3:
                ...
                transactions.append(
                    _make_txn(
                        ...
                        "Bonus",
                        TransactionType.INCOME,
                        f"Annual bonus - FY{year - 1}-{str(year)[2:]}",
                        len(transactions),
                    )
                )
    ...
    # Seed account classifications (upsert by user_id + account_name)
    for acct_name, acct_type in ACCOUNTS.items():
        existing = (
            db.query(AccountClassification)
            .filter_by(user_id=user_id, account_name=acct_name)
            .first()
        )
        if existing:
